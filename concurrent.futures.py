...
def multithreading_get_move_actions(board: np.array, weights):
    ...
    islands = board.island_active.copy()
    islands.remove(0)
    
    with concurrent.futures.ThreadPoolExecutor() as executor:
        perf_island = perf_counter()
        for island in islands:            
            debug(f'{island=}: {round((perf_counter() - perf_island) * 1000, 1)}')
            perf_island = perf_counter()
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                perf_unit = perf_counter()
                for row, col in zip(indices[0], indices[1]):
                    debug(f'({row}, {col}): {round((perf_counter() - perf_unit) * 1000, 1)}')
                    perf_unit = perf_counter()
                    scores = a_padded[row:row+3, col:col+3] * mask
                    if np.isnan(scores).all():
                        continue
    ...
    board.reset_island_number()
    debug(f'multithreading_get_move_actions: {round((perf_counter() - perf_start) * 1000, 1)}')
    return actions
...
def get_move_actions_for_unit(row, col, a_padded, mask):
    scores = a_padded[row:row+3, col:col+3] * mask
    if np.isnan(scores).all():
        return None
    row_offset, col_offset = [x - y for x, y in zip(np.unravel_index(np.nanargmax(scores), scores.shape), (1, 1))]
    return f"MOVE 1 {col} {row} {col + col_offset} {row + row_offset}"

def get_move_actions_for_island(board, weights, island):
    board.set_island_number(island)
    a = np.nansum([weight * min_max(getattr(board, feature)) for feature, weight in weights.items()], axis=0)
    a[board.tile_live == False] = np.nan
    a_padded = np.pad(a, 1, mode='constant', constant_values=np.nan)
    ...
    indices = np.where(board.units_me)
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = executor.map(get_move_actions_for_unit, indices[0], indices[1], repeat(a_padded), repeat(mask))
    results = list(results)
    return results if len(results) > 0 else [None]

def get_move_processpool(board, weights):
    results = []
    islands = board.island_active.copy()
    islands.remove(0)
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = executor.map(get_move_actions_for_island, repeat(board), repeat(weights), list(islands))    
    a = list(results)
    return [i[0] for i in a if i[0] is not None]
...
def main():
    ...
    while True:
        try:
            ...
            actions += get_move_processpool(board, move_weights)
