    ...
    while True:
        try:
            ...
            if board.zone_breach_op_unit_positions.sum() == 0:
                ...
                debug(f'{board.zone_breach_op_unit_positions.sum()} units breached my zone!')
                debug(f'{build_actions=}')
                debug(f'{spawn_actions=}')
                zone_breach_actions = build_actions + spawn_actions
                affordable_zone_breach_actions = zone_breach_actions[:my_matter // 10]
                debug(f'{affordable_zone_breach_actions=}')
            ...
            elif board.recycler_me.sum() / (1e-7 + (board.owner_me * board.live_active_tile).sum()) > .20:
                debug(f'More than 20% of my tiles are recyclers')
            else:
